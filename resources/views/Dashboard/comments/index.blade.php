    ...
    <div class="card mb-3" id="customersTable"
            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($comments->count() > 0 && $comments[0]->trashed())
                            {{ __('comments trash') }}
                        @else
                            {{ __('comments') }}
                        @endif
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('comments-create'))
                            <a href="{{ route('comments.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('comments.trashed') }}" class="btn btn-falcon-default btn-sm" type="button"><span
        ...
        <div class="card-body p-0">
            <div class="table-responsive scrollbar">
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach ($comments as $comment)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="joined align-middle py-2">{{ $comment->created_at }} <br>
                                        {{ interval($comment->created_at) }} </td>
                                    @if ($comment->trashed())
                                        <td class="joined align-middle py-2">{{ $comment->deleted_at }} <br>
                                            {{ interval($comment->deleted_at) }} </td>
                                    @endif
                                                ...
                                                <div class="bg-white py-2">
                                                    @if (
                                                        ...
                                                        <a class="dropdown-item"
                                                            href="{{ route('comments.restore', ['comment' => $comment->id]) }}">{{ __('Restore') }}</a>
                                                    @elseif(auth()->user()->hasPermission('comments-update'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('comments.edit', ['comment' => $comment->id]) }}">{{ __('Edit') }}</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('comments-delete') ||
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $comment->trashed() ? __('Delete') : __('Trash') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
                ...
                @else
                    <h3 class="p-4">{{ __('No commentsTo Show') }}</h3>
                @endif
            </div>
        </div>
